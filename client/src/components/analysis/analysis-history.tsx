...
export function AnalysisHistory() {
        ...
        try {
            ...
            setHistory((prev) =>
                prev.map((item) => {
                    ...
                    const previewSource =
                        (typeof detail.response_data?.final_response ===
                            "object" &&
                            detail.response_data.final_response !== null &&
                            typeof (
                                detail.response_data.final_response as Record<
                                    string,
                                    unknown
                                >
                        ...
                        (typeof detail.response_data?.quick_summary ===
                            "string" &&
                            detail.response_data.quick_summary) ||
                        (typeof detail.response_data?.quick_overview ===
                            "string" &&
                            detail.response_data.quick_overview) ||
                        (typeof detail.response_data?.analysis === "string" &&
                            detail.response_data.analysis) ||
                        item.preview ||
                        "";
    ...
    const isImageResponse = (
        value: unknown,
    ): value is ImageAnalysisResponse => {
    ...
    const isImageEnsembleResponse = (
        value: unknown,
    ): value is ImageAnalysisEnsembleResponse => {
        if (!value || typeof value !== "object") {
            return false;
        }
        ...
        const data = value as Record<string, unknown>;
        return (
            "final_response" in data &&
            typeof data.final_response === "object" &&
            data.final_response !== null &&
            isImageResponse(data.final_response) &&
            Array.isArray(data.model_outputs)
        );
    };

    const isSymptomsResponse = (
        value: unknown,
    ): value is SymptomsAnalysisResponse => {
        if (!value || typeof value !== "object") {
            return false;
        }
        const data = value as Record<string, unknown>;
    ...
    const getRenderableResult = (
        ...
        if (
            ...
            isImageEnsembleResponse(payload)
        ) {
            return payload;
        }
    ...
    const getLegacyResultText = (
        responseData: Record<string, unknown>,
    ): string | null => {
        const candidates = [
            responseData.final_response &&
            typeof responseData.final_response === "object"
                ? (responseData.final_response as Record<string, unknown>)
                      .quick_summary
                : undefined,
            ...
            {history.length === 0 && !isLoading ? (
                <Card>
                    <CardContent className="p-12 text-center">
                        ...
                        <CardDescription>
                            Your analysis history will appear here after you
                            perform image analysis, symptoms analysis, or get
                            care tips.
                        </CardDescription>
                    </CardContent>
                </Card>
